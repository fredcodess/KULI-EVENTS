...
export default function DesktopMenu({ menu }) {
  ...
  return (
    ...
    >
      ...
      {hasSubMenu && (
        <motion.div
          className="absolute left-0 top-full mt-2 bg-white text-accent rounded-lg p-4 shadow-lg z-50 min-w-[220px] space-y-3"
          initial="exit"
        ...
        >
          {menu.subMenu.map((item, i) => (
            <Link
              key={i}
              to={item.to}
              className="block hover:bg-white/10 p-2 rounded transition text-accent"
            >
